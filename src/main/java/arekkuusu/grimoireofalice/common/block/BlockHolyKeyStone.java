...
public class BlockHolyKeyStone extends BlockBase {
	...
	public BlockHolyKeyStone() {
		super(LibName.HOLY_KEY_STONE, Material.ROCK);
		setHardness(2.0F);
		...
		for (EnumFacing facing : EnumFacing.values()) {
			...
			if (world.getBlockLightOpacity(pos.offset(facing)) != 0) {
				switch (facing.getAxis()) {
					case X:
						x = pos.getX() + facing.getDirectionVec().getX() + d0;
						break;
					case Y:
						y = pos.getY() + facing.getDirectionVec().getY() + d0;
						break;
					case Z:
						z = pos.getZ() + facing.getDirectionVec().getZ() + d0;
						break;
				}
			}
