...
public class ShapedRecipe {
	...
	private Map<Character, Object> characters = new HashMap<>();
	...
	public void build() throws IllegalArgumentException {
		...
		if(row1 != null) {
			objects.add(row1);
			row1Chars = ImmutableSet.copyOf(ArrayUtils.toObject(row1.toCharArray()));
		}

		if(row2 != null) {
			objects.add(row2);
			row2Chars = ImmutableSet.copyOf(ArrayUtils.toObject(row2.toCharArray()));
		}

		if(row3 != null) {
			objects.add(row3);
			row3Chars = ImmutableSet.copyOf(ArrayUtils.toObject(row3.toCharArray()));
		}

		Set<Character> mappedCharacters = characters.keySet();
		...
		if(!row1Chars.isEmpty()) {
			throw new IllegalArgumentException("The first row has characters which are not mapped");
		}

		if(!row2Chars.isEmpty()) {
			throw new IllegalArgumentException("The second row has characters which are not mapped");
		}

		if(!row3Chars.isEmpty()) {
			throw new IllegalArgumentException("The third row has characters which are not mapped");
		}
		...
		ShapedOreRecipe recipe = new ShapedOreRecipe(out, objects);
